...
# match template
def match_template(image, temp):
    # image = cv2.resize(image, (320, 320))
    #temp = cv2.resize(temp, (76,76))
    #temp = np.float32(temp) / 255
    #temp = cv2.cvtColor(temp, cv2.COLOR_BGR2GRAY)
    result = cv2.matchTemplate(image, temp, cv2.TM_CCORR)
...
if __name__ == '__main__':
    ...
    apple_train = np.load('new_npy/apple_train.npy')
    lemon_train = np.load('new_npy/lemon_train.npy')
    orange_train = np.load('new_npy/orange_train.npy')
    ...
    for j in range(number):
            ...
            if true == 0:
                ...
                if cor_label == 0:
                    ...
                    #print(i)
                        ...
                        #image_c = cv2.rectangle(image_c, (x1, y1), (x1+w_e, y1+h_e), color = (0,0,255), thickness=4)
                    ...
                    #print(i)
                        ...
                        #image_c = cv2.rectangle(image_c, (x1, y1), (x1+w_e, y1+h_e), color = (50,255,255), thickness=4)
                    ...
                    #print(i)
                        ...
                        #image_c = cv2.rectangle(image_c, (x1, y1), (x1+w_e, y1+h_e), color = (50,200,255), thickness=4)
        ...
        cv2.imwrite(f'result/new_output/output_{j}.jpg', image)
        cv2.imwrite(f'result/new_out/out_{j}.jpg', img)
        # cv2.imshow("Img",img)
        # cv2.waitKey()
